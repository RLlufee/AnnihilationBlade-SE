package QWQ.QingYi.annihilationbladeex.visual;
...
public final class AnnihilationVisuals {
    ...
    public static void spawnBloodPrisonBurst(ServerLevel level, Vec3 center, double radius, RandomSource random) {
        level.sendParticles(ParticleTypes.FLASH, center.x, center.y, center.z, 2, 0.0D, 0.0D, 0.0D, 0.0D);
        level.sendParticles(ParticleTypes.CRIMSON_SPORE, center.x, center.y, center.z, 72, radius * 0.42D, radius * 0.34D, radius * 0.42D, 0.08D);
        level.sendParticles(ParticleTypes.DAMAGE_INDICATOR, center.x, center.y, center.z, 26, radius * 0.32D, radius * 0.26D, radius * 0.32D, 0.06D);
        level.sendParticles(ParticleTypes.DRIPPING_LAVA, center.x, center.y + 0.15D, center.z, 18, radius * 0.24D, radius * 0.18D, radius * 0.24D, 0.02D);
        spawnRing(level, center, X_AXIS, Z_AXIS, radius * 1.28D, ParticleTypes.DRAGON_BREATH, 54, 0.006D);
        spawnRing(level, center.add(0.0D, 0.18D, 0.0D), X_AXIS, UP, radius, ParticleTypes.CRIMSON_SPORE, 42, 0.008D);
        spawnRing(level, center.add(0.0D, 0.18D, 0.0D), Z_AXIS, UP, radius, ParticleTypes.ELECTRIC_SPARK, 34, 0.01D);

        for (int i = 0; i < 8; i++) {
            Vec3 dir = randomUnit(random);
            spawnLine(level, center, center.add(dir.scale(radius * (1.0D + random.nextDouble() * 0.85D))), i % 2 == 0 ? ParticleTypes.DRAGON_BREATH : ParticleTypes.ELECTRIC_SPARK, 7, 0.018D);
        }
    }
    ...
    public static void spawnBloodPrisonDash(ServerLevel level, Vec3 start, Vec3 end, RandomSource random) {
        Vec3 direction = safeNormalize(end.subtract(start), Z_AXIS);
        Vec3 right = safeNormalize(direction.cross(UP), X_AXIS);
        spawnLine(level, start, end, ParticleTypes.DRAGON_BREATH, 18, 0.025D);
        spawnLine(level, start.add(right.scale(0.62D)), end.add(right.scale(-0.4D)), ParticleTypes.CRIMSON_SPORE, 14, 0.02D);
        spawnLine(level, start.add(right.scale(-0.62D)), end.add(right.scale(0.4D)), ParticleTypes.ELECTRIC_SPARK, 10, 0.014D);
        for (int i = 0; i < 3; i++) {
            Vec3 mid = start.lerp(end, 0.25D + random.nextDouble() * 0.55D);
            Vec3 slash = right.scale(0.6D + random.nextDouble() * 0.75D).add(0.0D, random.nextDouble() * 0.7D - 0.25D, 0.0D);
            spawnLine(level, mid.subtract(slash), mid.add(slash), ParticleTypes.DAMAGE_INDICATOR, 5, 0.012D);
        }
        level.sendParticles(ParticleTypes.CRIMSON_SPORE, end.x, end.y, end.z, 34, 0.45D, 0.34D, 0.45D, 0.08D);
    }

    public static void spawnBloodPrisonDomainPulse(ServerLevel level, Vec3 center, double radius) {
        Vec3 foot = center.add(0.0D, 0.08D, 0.0D);
        spawnRing(level, foot, X_AXIS, Z_AXIS, radius, ParticleTypes.DRAGON_BREATH, 64, 0.004D);
        spawnRing(level, foot.add(0.0D, 0.04D, 0.0D), X_AXIS, Z_AXIS, radius * 0.72D, ParticleTypes.CRIMSON_SPORE, 42, 0.006D);
        spawnRing(level, foot.add(0.0D, 0.08D, 0.0D), X_AXIS, Z_AXIS, radius * 0.38D, ParticleTypes.DRIPPING_LAVA, 22, 0.002D);
        for (int i = 0; i < 4; i++) {
            double angle = TAU * i / 4.0D + Math.PI / 4.0D;
            Vec3 edge = foot.add(Math.cos(angle) * radius, 0.08D, Math.sin(angle) * radius);
            spawnLine(level, foot, edge, i % 2 == 0 ? ParticleTypes.CRIMSON_SPORE : ParticleTypes.ELECTRIC_SPARK, 12, 0.008D);
        }
    }
